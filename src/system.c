...
void sys_cycle(chip8* s) {
	...
	switch (identifier) {
		case 0x0: {
			if (s->opcode == 0x00E0) { // Clear: Clear The Display
				...
				if (s->stack[s->sptr] == 0) {
					LOG_E("invalid return from subroutine!");
				}
		...
		case 0x8: {
			...
			if (code == 0x0) { // Set Vx = Vy.
				...
				s->registers[Vx] = s->registers[Vx] | s->registers[Vy];
		...
		case 0xD: { // Display n-byte sprite starting at memory location I at (Vx, Vy), set VF = collision.
			...
			uint8_t rX = s->registers[Vx] % DISPLAY_WIDTH; // wraps around our sum, so 64 + 2 = 66 but since we only have a screen size of 64, 66 % 64 = 2.
			uint8_t rY = s->registers[Vy] % DISPLAY_HEIGHT; // same wrapping as above
			...
			for (uint16_t y = 0; y < height; ++y) {
				...
				for (uint16_t x = 0; x < SPRITE_WIDTH; ++x) {
					bool isBitAtX_Set = (s->ram[s->I + y] & (0x80 >> x)) != 0;
					if (isBitAtX_Set) {
						uint8_t pX = rX + x;
						uint8_t pY = rY + y;
	...
	// only increment Program counter if we didn't jump to a memory addr
	if (!didJump) sys_pctr_increment(s);
}
